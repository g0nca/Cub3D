...
static void     check_permissions(char **av)
{
    ...
    if (access(av[1], F_OK) == -1)
        print_error_and_exit_without_free("File doesn't exists", 1);
    if (access(av[1], R_OK) == -1)
        print_error_and_exit_without_free("Without Read Permissions", 1);
    if (stat(av[1], &info) == -1)
    {
        perror("stat");
        return ;
    ...
    if (S_ISDIR(info.st_mode))
        print_error_and_exit_without_free("Is a Directory", 1);
}
static void     check_extension(int ac, char **av)
{
    int len;
    ...
    if (ft_strlen(av[1]) > PATH_MAX)
        print_error_and_exit_without_free("Long Path is invalid", 1);
    if (av[1][0] == '.')
        print_error_and_exit_without_free("Invalid file name", 1);
    if (len < 5)
        print_error_and_exit_without_free("Invalid file name", 1);
    if (ft_strcmp(&av[1][len - 4], ".cub") != 0)
        print_error_and_exit_without_free("Invalid file extension", 1);
}
